...
class ComfyBaseService:
    ...
    def _parse_workflow_file(self, file_path: Path, source: str) -> Dict[str, Any]:
        ...
        # Check if it's a wrapper format (RunningHub, BizyAir, etc.)
        if "source" in content:
            if content.get("display_name"):
                workflow_info["display_name"] = content["display_name"]
            # Wrapper format: {"source": "runninghub", "workflow_id": "xxx", ...}
            if "workflow_id" in content:
                workflow_info["workflow_id"] = content["workflow_id"]
            if "api_type" in content:
                workflow_info["api_type"] = content["api_type"]
            if "endpoint" in content:
                workflow_info["endpoint"] = content["endpoint"]
            if "defaults" in content:
                workflow_info["defaults"] = content["defaults"]
            if "web_app_id" in content:
