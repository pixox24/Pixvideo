...
def test_quick_create_exposes_five_stage_navigation_and_section_anchors():
    ...
    for stage in ["内容", "分镜", "声音与画面", "核对并生成", "进度与结果"]:
        assert stage in quick_create
    for anchor in ["stage-content", "stage-storyboard", "stage-production", "stage-review"]:
        assert f'id="{anchor}"' in quick_create
    assert 'aria-current={activeStage === stage.id ? "step" : undefined}' in quick_create
...
def test_quick_create_autosaves_and_recovers_a_versioned_draft():
    ...
    assert "草稿已自动保存" in quick_create
...
def test_full_copy_render_respects_the_selected_split_rule_before_rebalancing():
    ...
    function_body = quick_create.split("const splitFullCopyDraftForRender", 1)[1].split("const buildScenesForRender", 1)[0]
    assert "splitDraftByCurrentRule(text)" in function_body
