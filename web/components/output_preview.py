...
def render_single_output(pixelle_video, video_params):
    ...
    minimax_model = video_params.get("minimax_model")
    minimax_emotion = video_params.get("minimax_emotion")
        ...
        if st.button(tr("btn.generate"), type="primary", use_container_width=True):
            ...
            try:
                ...
                if tts_mode == "local":
                    ...
                    video_params["tts_speed"] = tts_speed
                    if minimax_model:
                        video_params["minimax_model"] = minimax_model
                    if minimax_emotion:
                        video_params["minimax_emotion"] = minimax_emotion
                else:  # comfyui
                ...
                if result.storyboard.config.composition_mode == "plain_image":
                    video_width = result.storyboard.config.media_width
                    video_height = result.storyboard.config.media_height
                else:
...
def render_batch_output(pixelle_video, video_params):
    ...
    with st.container(border=True):
        ...
        if st.button(
            ...
            if shared_config["tts_inference_mode"] == "local":
            ...
            elif shared_config["tts_inference_mode"] == "minimax":
                tts_voice = video_params.get("tts_voice")
                tts_speed = video_params.get("tts_speed")
                minimax_model = video_params.get("minimax_model")
                minimax_emotion = video_params.get("minimax_emotion")
                ...
                if tts_speed:
                    shared_config["tts_speed"] = tts_speed
                if minimax_model:
                    shared_config["minimax_model"] = minimax_model
                if minimax_emotion:
                    shared_config["minimax_emotion"] = minimax_emotion
            else:  # comfyui
