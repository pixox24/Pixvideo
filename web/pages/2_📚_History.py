...
def render_grid_task_card(task: dict, pixelle_video):
    ...
    with st.container():
        ...
        columns = st.columns(4 if status in {"failed", "running"} else 3)
        ...
        if status in {"failed", "running"}:
            with columns[3]:
                if st.button("▶️", key=f"resume_open_{task_id}", help="继续生成", use_container_width=True):
                    st.session_state[f"detail_{task_id}"] = True
                    st.rerun()
...
def render_task_detail_modal(task_id: str, pixelle_video):
    ...
    if status in {"failed", "running"}:
        ...
        failed_frame_index = metadata.get("failed_frame_index")
        if failed_frame_index is not None:
            st.caption(f"失败位置: 分镜 {failed_frame_index + 1}")

        resume_progress = st.progress(0)
        resume_status = st.empty()

        def update_resume_progress(event: ProgressEvent):
            ...
            resume_status.text(message)
            resume_progress.progress(min(int(event.progress * 100), 99))

        if st.button("继续生成", type="primary", key=f"resume_task_{task_id}"):
            try:
                result = run_async(
                    pixelle_video.history.resume_task(
                        task_id,
                        pixelle_video,
                        progress_callback=update_resume_progress,
                    )
                )
                resume_progress.progress(100)
                resume_status.text(tr("status.success"))
